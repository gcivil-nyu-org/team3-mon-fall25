    ...
    def create(self, validated_data):
        ...
        for index, image_file in enumerate(images_data):
            try:
                ...
                # If any image upload fails, log the error and continue
                logger.error(
                    f"Failed to upload image for listing {listing.listing_id}: {e}"
                )
...
class ListingDetailSerializer(serializers.ModelSerializer):
    ...
    seller_member_since = serializers.DateTimeField(
        source="user.created_at", read_only=True
    )
    seller_active_listings_count = serializers.SerializerMethodField()
    seller_total_sold_count = serializers.SerializerMethodField()
    ...
    class Meta:
        ...
        fields = [
            ...
            "seller_display_name",
            "seller_member_since",
            "seller_active_listings_count",
            "seller_total_sold_count",
        ]
        read_only_fields = [
            "listing_id",
            "created_at",
    ...
    def get_seller_username(self, obj):
        """Return seller's netid or email as username"""
        if obj.user:
            return obj.user.netid if obj.user.netid else obj.user.email.split("@")[0]
        return None

    def get_seller_display_name(self, obj):
        """Return seller's full name or username"""
        if obj.user:
            if obj.user.first_name or obj.user.last_name:
                return f"{obj.user.first_name} {obj.user.last_name}".strip()
            return self.get_seller_username(obj)
        return None

    def get_seller_active_listings_count(self, obj):
        """Return count of seller's active listings"""
        if obj.user:
            return obj.user.listings.filter(status="active").count()
        return 0

    def get_seller_total_sold_count(self, obj):
        """Return count of seller's sold items"""
        if obj.user:
            return obj.user.listings.filter(status="sold").count()
        return 0
...
# Seller Public Profile Serializer
class SellerProfileSerializer(serializers.Serializer):
    """Serializer for public seller profile information"""
    ...
    listings = CompactListingSerializer(many=True)
