...
class ListingViewSet(
    ...
    def get_queryset(self):
        ...
        # Use select_related for detail view to avoid N+1 queries
...
class SellerProfileView(APIView):
    ...
    Endpoint: GET /api/v1/sellers/<username>/
    """
    ...
    def get(self, request, username):
        ...
        try:
            # Try to find user by netid first, then by email username
            user = User.objects.filter(
                Q(netid=username) | Q(email__istartswith=f"{username}@")
            ).first()
            ...
            listings = (
                Listing.objects.filter(user=user, status="active")
                .prefetch_related("images")
                .order_by("-created_at")
            )

            # Build seller profile data
            seller_username = user.netid if user.netid else user.email.split("@")[0]
            ...
            profile_data = {
                ...
                "active_listings_count": user.listings.filter(status="active").count(),
                "total_sold_count": user.listings.filter(status="sold").count(),
                "listings": listings,
            }

            serializer = SellerProfileSerializer(profile_data)
            return Response(serializer.data, status=status.HTTP_200_OK)
